 ...
 * 
 * Manages rate limits across multiple workers using Redis.
 * Ensures we never exceed Gemini Tier 3 limits (2000 RPM, 8M TPM).
 */

import { createClient, RedisClientType } from 'redis';
import { Logger } from '../util/log.js';
import { GEMINI_TIER_3_LIMITS } from '../../config/rate-limits.js';
...
export class SharedRateLimiter {
  ...
  constructor(logger: Logger, redisUrl?: string) {
    ...
    this.redis.on('error', (err) => {
      this.logger.error('rate_limiter', 'Redis error', err);
      this.isConnected = false;
    });
    
    this.redis.on('connect', () => {
      this.logger.info('rate_limiter', 'Connected to Redis');
  ...
  private async initializeCounters(): Promise<void> {
    ...
    if (!exists) {
      ...
      await this.logger.info('rate_limiter', 'Initialized rate limit counters');
    }
  }
  ...
  async recordApiCall(actualTokens: number): Promise<void> {
    ...
    await this.logger.debug('rate_limiter', 'Recorded API call', {
      tokens: actualTokens,
  ...
  async resetCounters(): Promise<void> {
    ...
    await this.logger.info('rate_limiter', 'Reset rate limit counters', {
      previousRPM: oldRPM,
      previousTPM: oldTPM,
      maxRPM: this.MAX_RPM,
      maxTPM: this.MAX_TPM
  ...
  startAutoReset(): NodeJS.Timeout {
    ...
    this.logger.info('rate_limiter', 'Started automatic rate limit reset (every 60s)');
  ...
  async registerWorker(workerInfo: WorkerInfo): Promise<void> {
    ...
    await this.logger.info('rate_limiter', 'Registered worker', {
  ...
  async unregisterWorker(workerId: string): Promise<void> {
    ...
    await this.logger.info('rate_limiter', 'Unregistered worker', {
      workerId
    });
  }
