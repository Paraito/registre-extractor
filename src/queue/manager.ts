...
export class QueueManager {
  ...
  async getQueueMetrics() {
    ...
    const [waiting, active, completed, failed] = await Promise.all([
      supabase.from('extraction_queue').select('id', { count: 'exact', head: true }).eq('status_id', EXTRACTION_STATUS.EN_ATTENTE),
      supabase.from('extraction_queue').select('id', { count: 'exact', head: true }).eq('status_id', EXTRACTION_STATUS.EN_TRAITEMENT),
      supabase.from('extraction_queue').select('id', { count: 'exact', head: true }).eq('status_id', EXTRACTION_STATUS.COMPLETE),
      supabase.from('extraction_queue').select('id', { count: 'exact', head: true }).eq('status_id', EXTRACTION_STATUS.ERREUR),
    ]);
